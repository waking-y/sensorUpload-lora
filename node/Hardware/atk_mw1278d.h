 ...
 * @version     V1.0
 * @date        2024-01-28
 * @brief       ATK-MW1278D模块驱动（已适配标准外设库）
...
#define ATK_MW1278D_AUX_GPIO_PIN            GPIO_Pin_4
#define ATK_MW1278D_MD0_GPIO_PORT           GPIOA
#define ATK_MW1278D_MD0_GPIO_PIN            GPIO_Pin_15

/* IO操作 */
#define ATK_MW1278D_AUX()                   GPIO_ReadInputDataBit(ATK_MW1278D_AUX_GPIO_PORT, ATK_MW1278D_AUX_GPIO_PIN)
#define ATK_MW1278D_MD0(x)                  do{ if(x) \
                                                GPIO_SetBits(ATK_MW1278D_MD0_GPIO_PORT, ATK_MW1278D_MD0_GPIO_PIN); \
                                                else \
                                                GPIO_ResetBits(ATK_MW1278D_MD0_GPIO_PORT, ATK_MW1278D_MD0_GPIO_PIN); \
                                            }while(0)
...
uint8_t atk_mw1278d_flash_config(atk_mw1278d_enable_t enable);                                      /* ATK-MW1278D模块参数掉电存储 */
uint8_t atk_mw1278d_default(void);                                                                  /* ATK-MW1278D模块恢复出厂设置 */
uint8_t atk_mw1278d_addr_config(uint16_t addr);                                                     /* ATK-MW1278D模块设备地址设置 */
uint8_t atk_mw1278d_tpower_config(atk_mw1278d_tpower_t tpower);                                     /* ATK-MW1278D模块发射功率设置 */
uint8_t atk_mw1278d_workmode_config(atk_mw1278d_workmode_t workmode);                               /* ATK-MW1278D模块工作模式设置 */
uint8_t atk_mw1278d_tmode_config(atk_mw1278d_tmode_t tmode);                                        /* ATK-MW1278D模块发射模式设置 */
uint8_t atk_mw1278d_wlrate_channel_config(atk_mw1278d_wlrate_t wlrate, uint8_t channel);            /* ATK-MW1278D模块无线速率和信道设置 */
uint8_t atk_mw1278d_wltime_config(atk_mw1278d_wltime_t wltime);                                     /* ATK-MW1278D模块唤醒时间设置 */
uint8_t atk_mw1278d_uart_config(atk_mw1278d_uartrate_t baudrate, atk_mw1278d_uartpari_t parity);    /* ATK-MW1278D模块串口参数设置 */
